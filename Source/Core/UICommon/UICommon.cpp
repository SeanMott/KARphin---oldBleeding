...
void SetUserDirectory(std::string custom_path)
{
 ...
 // bool appdata_found = SUCCEEDED(
  ...
  //DWORD local = true;
 // local != 0 || File::Exists(File::GetExeDirectory() + DIR_SEP "portable.txt");
 ...
 // if (local)  // Case 1-2
  //{
   user_path = File::GetExeDirectory() + DIR_SEP PORTABLE_USER_DIR DIR_SEP; //sets to local directory
 // }
...
//  {
    user_path = ROOT_DIR DIR_SEP EMBEDDED_USER_DIR DIR_SEP;//sets to local directory
 // }
 // else
 // {
